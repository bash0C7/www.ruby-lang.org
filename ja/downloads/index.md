...
FedoraはYUMパッケージマネジメントシステムを使っています。YUMを使うことでRubyをインストールできます。
...
インストールされるRubyのバージョンは、一般的に、Fedoraがリリースされた時の最新の安定版Rubyの最新バージョンです（例えば、Fedora 20ではRuby 2.0.0がインストールされ、Fedora 21ではおそらくRuby 2.1.0が提供されるでしょう）。
